...
prune.tests.DA <- function(tests, predictor, paired, covars, relative){
  ...
  if(length(levels(as.factor(predictor))) > 2){
    tests <- tests[tests %in% c("qpo","poi","neb","erq","ds2","lim","lli","lli2","aov","lao","lao2","kru","lrm","llm","llm2","spe","pea","zig")]
  } else {
  ...
  if(is.numeric(predictor)){
    tests <- tests[tests %in% c("qpo","poi","neb","erq","ds2","lim","lli","lli2","lrm","llm","llm2","spe","pea")]
  } else {
  ...
  if(relative == FALSE){
    tests <- tests[!tests %in% c("poi","qpo","ltt2","neb","erq","ere","msf","zig","bay","ds2","adx","lli2","lao2","llm2","rai")]
  }
  
  # Only include if covars are present
  if(!is.null(covars)){
    tests <- tests[tests %in% c("qpo","poi","ds2","neb","erq","zig","lrm","llm","llm2","lim","lli","lli2","aov","lao","lao2")]
  }
