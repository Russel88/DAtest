...
prune.tests.DA <- function(tests, predictor, paired, covars, relative){
  ...
  if(length(levels(as.factor(predictor))) > 2){
    tests <- tests[tests %in% c("qua","fri","znb","zpo","vli","qpo","poi","neb","erq","erq2","ds2","lim","lli","lli2","aov","lao","lao2","kru","lrm","llm","llm2","spe","pea","zig")]
  } else {
  ...
  if(relative == FALSE){
    tests <- tests[!tests %in% c("znb","zpo","vli","poi","qpo","ltt2","neb","erq","ere","ere2","erq2","msf","zig","bay","ds2","adx","lli2","lao2","llm2","rai")]
  }
