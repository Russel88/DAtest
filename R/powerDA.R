...
powerDA <- function(data, predictor, paired = NULL, covars = NULL, test = NULL, effectSizes = c(2,4,8,16,32), alpha = 0.1, p.adj = "fdr", R = 5, relative = TRUE, k = NULL, cores = (detectCores()-1), rng.seed = 123, args = list(), out.all = NULL, core.check = TRUE, verbose = TRUE){
  ...
  results <- foreach(i = tests.par, .options.snow = opts) %dopar% {
    ...
    res.sub <- tryCatch(switch(i,
                               ...
                               adx = do.call(get(noquote(paste0("DA.",i))),c(list(count_tables[[what.run]],rands[[run.no]],p.adj),adx.DAargs)),
  ...
  # Split aldex
  if(test == "adx"){
    results.t <- lapply(results, function(x) x[,c(1:8,10,12,13)])
    results.w <- lapply(results, function(x) x[,c(1:7,9,11:13)])
