...
DA.ltt <- function(data, outcome, paired = NULL, relative = TRUE, p.adj = "fdr", delta = 1, testStat = function(case,control){log((mean(case)+1)/(mean(control)+1))}, testStat.pair = function(case,control){mean(log((case+1)/(control+1)))}, ...){
