...
spikein <- function(count_table, outcome, spikeMethod = "mult", effectSize = 2, k, num.pred = FALSE, relative = TRUE){
  ...
  if(!min(count_table) < 0){
    ...
    # Choose features to spike
    count_abundances <- sort(rowMeans(count_table))
    
    lower_tert <- names(count_abundances[count_abundances < quantile(count_abundances,1/3)])
    mid_tert <- names(count_abundances[count_abundances >= quantile(count_abundances,1/3) & count_abundances < quantile(count_abundances,2/3)])
    upper_tert <- names(count_abundances[count_abundances >= quantile(count_abundances,2/3)])
  ...
  if(spikeMethod == "mult"){
    ...
    if(num.pred){
      ...
      count_table[spike_feature_index,] <- count_table[spike_feature_index, ] * (effectSize ^ scale(outcome))
    } else {
  ...
  if(spikeMethod == "add"){
    ...
    if(num.pred){
        addcounts <- count_table[spike_feature_index, ] * t(scale(outcome) %*% t(rep(log2(effectSize),length(spike_feature_index))))
        count_table[spike_feature_index, ] <- count_table[spike_feature_index, ] + addcounts     
      } else {
        addcounts <- count_table[spike_feature_index, outcome == 1] * log2(effectSize)
        count_table[spike_feature_index, outcome == 1] <- count_table[spike_feature_index, outcome == 1] + addcounts
      }
