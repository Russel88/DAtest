...
#' 
#' # Running limma
#' res <- DA.vli(data = mat, predictor = pred)
#' @export

DA.vli <- function(data, predictor, paired = NULL, covars = NULL, out.all = NULL, p.adj = "fdr", coeff = 2, allResults = FALSE, ...){
  
  ok <- tryCatch({
    if(is.null(paired)){loadNamespace(c("edgeR","limma"))} else {loadNamespace(c("edgeR","limma","statmod"))}
    TRUE
  ...
  if (ok){
    ...
    voom.args <- limma.args[names(limma.args) %in% names(formals(limma::voom))]
    ...
    voo <- do.call(limma::voom,c(list(count_table, design, lib.size=colSums(count_table)*nf),voom.args))
    
    # Fit linear model
    if(is.null(paired)){
      fit <- do.call(limma::lmFit,c(list(voo, design),lmFit.args))
    } else {
      dupcor <-  limma::duplicateCorrelation(count_table, design, block = paired)
      fit <- do.call(limma::lmFit,c(list(voo, design, block = paired, correlation = dupcor$cor),lmFit.args))
    }
    ...
    if(!is.null(paired)){
      stop("limma, edgeR and statmod packages required")
    } else {
      stop("limma and edgeR packages required")
    }
  }
